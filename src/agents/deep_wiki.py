...
async def generate_deep_wiki(chat_history: str = "", user_query: str = "") -> str:
    ...
    try:
        ...
        async with credential, project_client:
            ...
            functions = AsyncFunctionTool({vector_search.vector_search})
            ...
            # Retrieve all messages and find the latest assistant message
            # We consume the full iterator to ensure proper cleanup
            response = None
            messages_iterator = project_client.agents.messages.list(thread_id=thread.id)
            async for message in messages_iterator:
                if message.role == "assistant":
                    # Get the text content from the message
                    if message.content and len(message.content) > 0:
                        # Keep updating response to get the latest one
                        response = str(message.content[0].text.value)
            
            # Explicitly delete the iterator reference to trigger cleanup
            del messages_iterator
            
            # Store response before cleanup to avoid holding references
            ...
            logger.info("Deleting thread: %s", thread.id)
            await project_client.agents.threads.delete(thread.id)
            logger.info("Thread deleted successfully")
            
            logger.info("Deleting agent: %s", agent.id)
            await project_client.agents.delete_agent(agent.id)
            logger.info("Agent deleted successfully")
            
            logger.info("Wiki documentation generated. Response length: %d", len(final_response))
